...
<form style="margin-top:3rem;" method="post" id="dataForm">
    <div class="container">
        <h1 class="text-center my-3" style="color:#154782">Lead Traveler Details Form</h1>
        ...
        <div class="form-group">
            ...
            <input type="text" id="packageId" value="<?php echo $packageId; ?>" hidden>
            <input type="text" id="cabinId" hidden value="<?php echo $cabinId ?>">
        ...
        <button type="submit" id="submitBtn" class="btn" style="color:#48C7F4; background-color:#004585;">Submit</button>
    </div>
</form>
<script>
document.addEventListener('DOMContentLoaded', function() {
    var storedData = sessionStorage.getItem('leadformData');
    if (storedData) {
        var leadformData = JSON.parse(storedData);
        document.getElementById('options').value = leadformData.title;
        document.getElementById('fname').value = leadformData.fname;
        document.getElementById('lname').value = leadformData.lname;
        document.getElementById('address1').value = leadformData.address1;
        document.getElementById('address2').value = leadformData.address2;
        document.getElementById('postno').value = leadformData.postno;
        document.getElementById('city').value = leadformData.city;
        document.getElementById('state').value = leadformData.state;
        document.getElementById('country').value = leadformData.country;
        document.getElementById('phone').value = leadformData.phone;
        document.getElementById('email').value = leadformData.email;
        document.getElementById('veg').checked = leadformData.veg;
        document.getElementById('lactose').checked = leadformData.lactose;
        document.getElementById('vegan').checked = leadformData.vegan;
        document.getElementById('gluten').checked = leadformData.gluten;
        document.getElementById('infants_details').value = leadformData.infants_details;
        document.getElementById('other_request').value = leadformData.other_request;
    }
});
...
document.getElementById('submitBtn').addEventListener('click', function(event) {
    ...
    var entryIndex = existingEntries.findIndex(entry => entry.packageId === packageId);

    if (entryIndex !== -1) {
        // Update existing entry
        existingEntries[entryIndex] = leadformData;
    } else {
        // Add new entry
        existingEntries.push(leadformData);
    }

    sessionStorage.setItem('usersData', JSON.stringify(existingEntries));
    console.log('Form data saved to session storage:', leadformData);
    storeSessionAndRedirect(leadformData, existingEntries);
});

function storeSessionAndRedirect(leadformData, existingEntries) {
    var select_type = document.getElementById('select_type').value;
    var Dateurl = document.getElementById('Date').value;
    var url = `selectGuest.php?packageId=${leadformData.packageId}&cabinId=${leadformData.cabinId}&select_type=${select_type}&Date=${Dateurl}`; 
    fetch('storeLeadformsession.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ leadformData: leadformData, existingEntries: existingEntries })
    })
